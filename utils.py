...
def record_net_data_stats(y_train, net_dataidx_map, logdir):
    ...
    for net_i, dataidx in net_dataidx_map.items():  # 将字典中的键值对用列表形式返回，key=net_i,dataidx=valua
...
def partition_data(dataset, datadir, logdir, partition, n_parties, beta=0.4):
    ...
    if partition == "homo":
        ...
        if num == 10:
            ...
            times=[0 for i in range(K)]     # 统计每类出现的次数
            contain=[]      # 生成每个用户拥有的类
    ...
    traindata_cls_counts = record_net_data_stats(y_train, net_dataidx_map, logdir)  # 以字典的形式返回标签集中每类标签的数量
